...
deathstar_radius = 80
deathstar_health = 750
#FRED DIFFICULTY: 10
#EASY DIFFICULTY: 100
#MEDIUM DIFFICULTY: 250
#HARD DIFFICULTY 400
#The force is against you: 750

# Score and Timers
score = 0
font = pygame.font.Font(None, 36)
start_time = pygame.time.get_ticks()  # Game start time
initial_timer_limit = 20 * 1000  # 20 seconds initial timer
wipeout_timer_limit = 30 * 1000  # 30 seconds wipeout timer
wipeout_start_time = None  # Will be set after initial timer finishes
...
while running:
    ...
    initial_time_remaining = max(0, (initial_timer_limit - elapsed_time) // 1000)  # Initial 20s timer
    
    # Start wipeout timer after initial timer finishes
    if elapsed_time >= initial_timer_limit and wipeout_start_time is None:
        wipeout_start_time = current_time
    
    # Calculate wipeout timer if it has started
    ...
    for laser in lasers[:]:
        laser[1] += laser_speed
        # Only damage Death Star after initial timer expires
    ...
    screen.blit(initial_text, (WIDTH - 150, 10))  # Top right
